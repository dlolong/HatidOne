...
- Local restart: all enabled service health checks passed, followed by another successful 27-check HTTP run. Studio and its metadata API are now opt-in to reduce memory use alongside other projects in the shared 3 GB Docker VM; see the runbook for enabling them.
