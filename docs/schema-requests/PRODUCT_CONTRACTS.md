...
`request_driver_application() -> void` (migration `0010_driver_application_journey.sql`) creates only the authenticated active passenger/driver’s own pending, offline application, retaining their current role and passenger bookings. A profile lock and unique user ID make retries/concurrent requests idempotent; existing submitted, reviewed, and suspended applications are never reset. Applicant edit/submit RPCs serialize against review and permit only own editable pending/rejected applications. Only trusted `admin_review_driver(..., 'verified', ...)` promotes a submitted passenger applicant to the existing driver role. Driver operational eligibility remains separately enforced. Approval waits while an applicant has active passenger bookings so the role transition cannot strand those trips. Booking insertion and approval share a profile lock; a request cannot race across the role change. Historical own passenger rides remain readable after approval. The existing role model still restricts new passenger booking mutations to passenger accounts.
...
`supabase/seed.sql` is local only and explicitly enables demo configuration. Each fixture user has an independent random password; local backend preparation stores per-email credentials in ignored `.local-backend/test-accounts.json` (0600), while the SQL-only test harness uses per-account random fallback. No shared fixed credential is supported. Accounts: passenger, driver, pending-driver, admin, fleet, partner, corporate, rider, backup-driver, outsider at `@hatidone.test`. Fictional document metadata is seeded without claiming actual reviewed documents or uploading bytes. Replace demo metadata with real private document uploads when testing storage downloads. Do not run seed against production.
