...
const destinations: Record<AppRole, { label: string; href: string }[]> = {
  ...
  admin: [{ label: 'Overview', href: '/admin' }, { label: 'Dispatch', href: '/admin/dispatch' }, { label: 'Operations', href: '/admin/operations' }, { label: 'Business', href: '/organizations' }],
...
export function AppNavigation({ role }: { role: AppRole }) {
  ...
  return <nav className="primary-nav" aria-label={`${role === 'admin' ? 'Operations' : 'Main'} navigation`}>{destinations[role].map(item => {
    const active = path === item.href || item.href === '/organizations' && path.startsWith('/organizations/') || item.href === '/history' && path.startsWith('/booking/');
