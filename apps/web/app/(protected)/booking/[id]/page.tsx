...
export default async function BookingDetailPage({ params, searchParams }: BookingPageProps) {
  ...
  return (
    <div className="booking-page">
      ...
      {driver && <section className="detail-card"><h2>Your assigned driver</h2><p>{driver.name} · {driver.verification_status}</p><p>{driver.brand} {driver.model} · {driver.color} · {driver.plate_number}</p><p>{driver.rating_count ? `${driver.rating} rating from ${driver.rating_count} reviews` : 'Rating history still building'}</p>{pin && <p>Passenger PIN: <strong className="pickup-pin">{pin}</strong>. Share with your driver only when you are ready to start.</p>}</section>}
      ...
      <div className="booking-detail-grid">
        ...
        <section className="detail-card fare-card"><h2>Fare estimate</h2><strong>{formatPeso(booking.estimated_fare)}</strong><p>Local fare estimate before unverified tolls. Confirm tolls and pickup details with your driver.</p></section>
